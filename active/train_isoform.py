...
# ============================================================
# Metrics (regression: R², Pearson R²)
# ============================================================
...
# (Regression task: uncertainty estimated from prediction variance)
#
# For regression, the model outputs a single scalar per sample.
# - Entropy doesn't apply directly. We use predictive variance instead.
# - Margin doesn't apply. We use the same variance-based score.
# - BALD: variance of MC-Dropout predictions.
# - For "entropy" and "margin", we fall back to MC-Dropout variance
#   as the uncertainty measure (standard for regression AL).
# ============================================================
...
@torch.no_grad()
def _get_pool_predictions(trainer, pool_dataset, data_collator):
    """Single forward pass. Returns predictions (N,)."""
...
def _get_pool_predictions_mc(trainer, pool_dataset, data_collator, num_mc_samples: int):
    ...
    for _ in range(num_mc_samples):
        ...
        for batch in loader:
            batch = {k: v.to(device) for k, v in batch.items() if k != "labels"}
            outputs = trainer.model(**batch)
            sample_preds.append(outputs.logits.squeeze(-1).cpu())
        all_mc_preds.append(torch.cat(sample_preds, dim=0).numpy())

    trainer.model.eval()
    return np.stack(all_mc_preds, axis=0)  # (T, N)
...
def acquire_entropy(trainer, pool_dataset, data_collator, budget: int,
    ...
    return np.argsort(variance)[::-1][:budget]
...
def acquire_margin(trainer, pool_dataset, data_collator, budget: int,
    ...
    Smaller std = more confident. Pick samples with highest std (least confident).
    (Equivalent ranking to variance, but semantically maps to 'margin' idea.)
    """
    mc_preds = _get_pool_predictions_mc(trainer, pool_dataset, data_collator, num_mc_samples)
    std = mc_preds.std(axis=0)  # (N,)
    return np.argsort(std)[::-1][:budget]
...
def acquire_bald(trainer, pool_dataset, data_collator, budget: int,
    ...
    I[y;w|x] ≈ Var_w[E[y|x,w]] = variance of MC predictions.
    """
    mc_preds = _get_pool_predictions_mc(trainer, pool_dataset, data_collator, num_mc_samples)
    variance = mc_preds.var(axis=0)  # (N,)
    return np.argsort(variance)[::-1][:budget]
...
def acquire_variation_ratio(trainer, pool_dataset, data_collator, budget: int,
                            num_mc_samples: int = 10, **kwargs) -> np.ndarray:
    """
    For regression, variation ratio is not directly defined.
    We use coefficient of variation (std / |mean|) as an analog:
...
# ============================================================
# Checkpoint helpers (same as SS/SpliceAI v3)
# ============================================================
...
def train():
    ...
    for data_test in data_test_list:
        print(f"Loading test set: {data_test}")
        test_datasets[data_test] = SupervisedDataset(
            tokenizer=tokenizer, args=training_args,
            data_path=os.path.join(data_args.data_path, data_test),
            kmer=data_args.kmer,
        )
    ...
    for round_idx in range(start_round, num_rounds):
        ...
        # ---- Evaluate on val + all test sets ----
        val_results = trainer.evaluate(eval_dataset=val_dataset)
        test_results_by_set = {}
        for data_test in data_test_list:
            test_results_by_set[data_test] = trainer.evaluate(eval_dataset=test_datasets[data_test])
        ...
        print(f"  Val pearson_r²: {val_results.get('eval_pearson_r_squared', 'N/A')}")
        for data_test in data_test_list:
            tr = test_results_by_set[data_test]
            print(f"  [{data_test}] pearson_r²={tr.get('eval_pearson_r_squared', 'N/A')}, "
                  f"r²={tr.get('eval_r^2', 'N/A')}")
        ...
        for data_test in data_test_list:
            with open(os.path.join(results_path, f"{data_test}_results.json"), "w") as f:
                json.dump(test_results_by_set[data_test], f, indent=4)
    ...
    for r in aggregate_summary:
        ...
        for data_test in data_test_list:
            ...
            pr2 = tr.get("eval_pearson_r_squared", "N/A")
            line += f", {data_test}_pr²={pr2}"
        print(line)
