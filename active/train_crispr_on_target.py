...
class TrainingArguments(transformers.TrainingArguments):
    ...
    patience: int = field(default=20, metadata={"help": "Early stopping patience"})  # CHANGE: configurable patience (was hardcoded 20)
...
# Note: single-input (one sequence per sample), 2-column CSV.
# Data is pre-tokenized into input_ids/attention_mask tensors.
# ============================================================

class SupervisedDataset(Dataset):
    """Dataset for supervised fine-tuning (single-sequence regression)."""
...
# Note: simple collator — stacks pre-tokenized tensors.
# No struct mask, no flattening. Labels are (B,) scalars.
# ============================================================
...
# So there is a guaranteed 1:1 correspondence between pool samples
# and logit entries — no IndexError risk from mask flattening.
#
# We use per-sample MC-Dropout scoring for consistency and include
# safety assertions.
# ============================================================
...
def _get_pool_predictions_mc(trainer, pool_dataset, data_collator, num_mc_samples: int):
    ...
    For this single-sequence regression task, model returns logits of shape
...
def acquire_variation_ratio(trainer, pool_dataset, data_collator, budget: int,
    ...
    (std / |mean|) as analog of variation ratio.
    """
    mc_preds = _get_pool_predictions_mc(trainer, pool_dataset, data_collator, num_mc_samples)
...
# Note: uses *ForSequenceClassification model classes (not *ForCRISPROffTarget).
# Note: no tokenizer arg — these SequenceClassification models don't take it.
# ============================================================
...
#   - Model classes: *ForSequenceClassification
#   - build_model() does NOT take tokenizer
#   - Removed train_fraction subsampling (replaced by AL scheduling)
# ============================================================
...
def train():
    ...
    for round_idx in range(start_round, num_rounds):
        ...
        # CHANGE: Build fresh model each round (no tokenizer arg for SequenceClassification)
