...
class SupervisedDataset(Dataset):
    """Dataset for supervised fine-tuning (sequence classification)."""
    ...
    def __init__(self, data_path: str, args,
        ...
        if len(data[0]) == 2:
            ...
            raise ValueError(f"Data format not supported. Expected 2 columns, got {len(data[0])}")
...
# ============================================================
# Metrics (classification: accuracy, F1, MCC, precision, recall)
# ============================================================
...
def _get_pool_logits(trainer, pool_dataset, data_collator):
    """Single forward pass. Returns logits (N, C)."""
    ...
    return torch.cat(all_logits, dim=0)  # (N, C)


@torch.no_grad()
def _get_pool_logits_mc(trainer, pool_dataset, data_collator, num_mc_samples: int):
    """MC-Dropout forward passes. Returns (num_mc_samples, N, C)."""
    loader = torch.utils.data.DataLoader(
        pool_dataset,
        batch_size=trainer.args.per_device_eval_batch_size,
        collate_fn=data_collator,
    ...
    return torch.stack(all_mc_logits, dim=0)  # (T, N, C)
...
def acquire_entropy(trainer, pool_dataset, data_collator, budget: int, **kwargs) -> np.ndarray:
    ...
    """
    logits = _get_pool_logits(trainer, pool_dataset, data_collator)  # (N, C)
    probs = torch.softmax(logits, dim=-1).numpy()
    eps = 1e-10
    entropy = -np.sum(probs * np.log(probs + eps), axis=-1)  # (N,)
    return np.argsort(entropy)[::-1][:budget]


def acquire_margin(trainer, pool_dataset, data_collator, budget: int, **kwargs) -> np.ndarray:
    """
    Select samples with smallest margin between top-2 class probabilities.
    ...
    return np.argsort(margin)[:budget]  # ascending: pick smallest margins
...
def acquire_bald(trainer, pool_dataset, data_collator, budget: int,
    ...
    mc_probs = torch.softmax(mc_logits, dim=-1).numpy()  # (T, N, C)

    eps = 1e-10
    mean_probs = mc_probs.mean(axis=0)  # (N, C)
    predictive_entropy = -np.sum(mean_probs * np.log(mean_probs + eps), axis=-1)  # (N,)

    per_sample_entropy = -np.sum(mc_probs * np.log(mc_probs + eps), axis=-1)  # (T, N)
...
def acquire_variation_ratio(trainer, pool_dataset, data_collator, budget: int,
                            num_mc_samples: int = 10, **kwargs) -> np.ndarray:
    """
    Variation ratio: 1 - (count of mode class) / T.
    Higher = more disagreement among MC samples.
    """
    mc_logits = _get_pool_logits_mc(trainer, pool_dataset, data_collator, num_mc_samples)
    mc_preds = mc_logits.argmax(dim=-1).numpy()  # (T, N)
    T = mc_preds.shape[0]

    from scipy import stats
    mode_counts = stats.mode(mc_preds, axis=0, keepdims=False).count  # (N,)
    variation_ratio = 1.0 - mode_counts / T  # (N,)
    return np.argsort(variation_ratio)[::-1][:budget]
...
# ============================================================
# CHANGE: Model Builder (extracted from train() for per-round rebuild)
# ============================================================
...
def train():
    ...
    for round_idx in range(start_round, num_rounds):
        ...
        # load_best_model_at_end=True ensures trainer.model has best-val weights
        ...
        print(f"  Val  accuracy: {val_results.get('eval_accuracy', 'N/A')}, "
              f"F1: {val_results.get('eval_f1', 'N/A')}")
        print(f"  Test accuracy: {test_results.get('eval_accuracy', 'N/A')}, "
              f"F1: {test_results.get('eval_f1', 'N/A')}, "
              f"MCC: {test_results.get('eval_matthews_correlation', 'N/A')}")
    ...
    for r in aggregate_summary:
        test_acc = r["test_results"].get("eval_accuracy", "N/A")
        test_f1 = r["test_results"].get("eval_f1", "N/A")
        test_mcc = r["test_results"].get("eval_matthews_correlation", "N/A")
        print(f"  Round {r['round']}: frac={r['labeled_fraction']:.2f}, "
              f"size={r['labeled_size']}, epochs={r['scaled_epochs']}, "
              f"acc={test_acc}, F1={test_f1}, MCC={test_mcc}")
