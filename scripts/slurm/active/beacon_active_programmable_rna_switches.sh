#!/usr/bin/env bash
# submit_beacon_active_learning_programmable_rna_switches.sh
# Submit active learning experiments for BEACON on ProgrammableRNASwitches
set -euo pipefail
...
for entry in "${MODELS[@]}"; do
  ...
  for AL_STRATEGY in "${AL_STRATEGIES[@]}"; do
    ...
    JOB_NAME="al_${MODEL_NAME}_PRSwitches_${AL_STRATEGY}"
...
# ---------------------------------------------------------------
# ProgrammableRNASwitches — Active Learning
# ---------------------------------------------------------------
...
\$EXEC_PREFIX active/train_programmable_rna_switches.py \
