#!/usr/bin/env bash
# submit_beacon_active_learning_structural_imputation.sh
# Submit active learning experiments for BEACON on Structural Score Imputation
set -euo pipefail
...
for entry in "${MODELS[@]}"; do
  ...
  for AL_STRATEGY in "${AL_STRATEGIES[@]}"; do
    ...
    JOB_NAME="al_${MODEL_NAME}_StructImputation_${AL_STRATEGY}"
...
# ---------------------------------------------------------------
# Structural Score Imputation — Active Learning
# ---------------------------------------------------------------
...
\$EXEC_PREFIX active/train_structural_score_imputation.py \
