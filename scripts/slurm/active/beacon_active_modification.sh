#!/usr/bin/env bash
# submit_beacon_modification_active_learning.sh
set -euo pipefail
...
LOG_DIR=${CODE_ROOT}/logs/beacon_al_modification
mkdir -p "$LOG_DIR"
...
# Base epochs & patience — match original Modification training config
AL_BASE_EPOCHS=30
AL_PATIENCE=20
...
for entry in "${MODELS[@]}"; do
  ...
  for AL_STRATEGY in "${AL_STRATEGIES[@]}"; do
    ...
    JOB_NAME="al_${MODEL_NAME}_Modification_${AL_STRATEGY}"
...
echo "=== Modification Active Learning started on \$(hostname) at \$(date) ==="
...
\$EXEC_PREFIX active/train_modification.py \
  --model_name_or_path \${PRETRAINED_PATH} \
  --data_path \$DATA_PATH \
  --data_train_path train.csv --data_val_path val.csv --data_test_path test.csv \
  --run_name \${MODEL_NAME}_\${task}_seed\${SEED}_lr\${lr}_AL_\${AL_STRATEGY} \
...
echo "=== Modification Active Learning for \$MODEL_NAME (\$AL_STRATEGY) completed at \$(date) ==="
EOT
...
echo "All Modification Active Learning jobs submitted."