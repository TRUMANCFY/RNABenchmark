#!/usr/bin/env bash
# submit_beacon_active_learning_mean_ribosome_loading.sh
# Submit active learning experiments for BEACON on MeanRibosomeLoading
set -euo pipefail
...
for entry in "${MODELS[@]}"; do
  ...
  for AL_STRATEGY in "${AL_STRATEGIES[@]}"; do
    ...
    JOB_NAME="al_${MODEL_NAME}_MRL_${AL_STRATEGY}"
...
# ---------------------------------------------------------------
# MeanRibosomeLoading — Active Learning
# ---------------------------------------------------------------
...
\$EXEC_PREFIX active/train_mean_ribosome_loading.py \
