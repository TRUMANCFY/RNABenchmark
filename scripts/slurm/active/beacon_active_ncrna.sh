#!/usr/bin/env bash
# submit_beacon_active_learning_ncrna.sh
# Submit active learning experiments for BEACON on NoncodingRNAFamily
set -euo pipefail
...
for entry in "${MODELS[@]}"; do
  ...
  for AL_STRATEGY in "${AL_STRATEGIES[@]}"; do
    ...
    JOB_NAME="al_${MODEL_NAME}_ncRNA_${AL_STRATEGY}"
...
# ---------------------------------------------------------------
# NoncodingRNAFamily — Active Learning
# ---------------------------------------------------------------
...
\$EXEC_PREFIX active/train_ncrna.py \
