...
run_benchmark() {
    ...
    local EXEC_PREFIX="env CUDA_VISIBLE_DEVICES=$GPU_ID WANDB_DISABLED=true torchrun --nproc_per_node=$nproc_per_node --master_port=$master_port"
    ...
    OUTPUT_PATH=./outputs_test/ft/rna-all/${task}/${MODEL_NAME}
...
# Loop over all models, always using GPU 2
for (( i=0; i<total_models; i++ )); do
    ...
    # Run Benchmark with specific GPU ID (always 2)
